...
{
	plist::any_t plist = plist::parse_ascii(src);
	return std::get< std::vector<plist::any_t> >(&plist);
}

static std::vector<plist::any_t> parse_array (std::string const& src)
{
	plist::any_t plist = plist::parse_ascii(src);
	if(std::vector<plist::any_t> const* res = std::get< std::vector<plist::any_t> >(&plist))
		return *res;
...
{
	plist::any_t plist = plist::parse_ascii(src);
	return std::get< std::map<std::string, plist::any_t> >(&plist);
}

static std::map<std::string, plist::any_t> parse_dict (std::string const& src)
{
	plist::any_t plist = plist::parse_ascii(src);
	if(std::map<std::string, plist::any_t> const* res = std::get< std::map<std::string, plist::any_t> >(&plist))
		return *res;
...
void test_bool ()
{
	...
	OAK_ASSERT(std::get< std::vector<plist::any_t> >(&mixedPlist));
...
void test_dictionary_keys ()
{
	...
	integerMap["42"] = 1;
	integerMap["80"] = 2;
	OAK_ASSERT_EQ(to_s(plist::parse_ascii("{ 42 = 1; 80 = 2; }")), to_s(plist::to_plist(integerMap)));

	std::map<std::string, std::string> booleanMap;
	booleanMap["1"] = std::string("true");
	booleanMap["0"] = std::string("false");
	OAK_ASSERT_EQ(to_s(plist::parse_ascii("{ :true = true; :false = false; }")), to_s(plist::to_plist(booleanMap)));

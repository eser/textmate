...
namespace
{
	...
	struct fits_single_line
	{
		...
		bool operator() (plist::array_t const& array) const
		{
			...
			{
				singleLineItems = singleLineItems && std::visit(*this, it);
				compositeItems = compositeItems || std::visit(is_composite(), it);
			}
		...
		bool operator() (plist::dictionary_t const& dict) const
		{
			...
			for(auto const& it : dict)
				res = res && std::visit(*this, it.second);
			return res;
		}
	};
}
		...
		std::string operator() (plist::array_t const& array) const
		{
			...
			else if(singleLine || fits_single_line()(array))
			{
				...
				for(auto const& it : array)
				{
					...
					res += std::visit(pretty(flags | plist::kSingleLine, key_compare), it);
				}
			...
			else
			{
				for(auto const& it : array)
				{
					if(!res.empty())
						res += "\n";
					res += indent_string() + '\t';
					res += std::visit(pretty(flags, key_compare, indent+1), it);
					res += ",";
				}
				res = "\n" + res + "\n" + indent_string();
			}
		...
		std::string operator() (plist::dictionary_t const& dict) const
		{
			...
			else if(singleLine || fits_single_line()(dict))
			{
				...
				for(auto const& it : dict)
				{
					...
					std::string const& value = std::visit(pretty(flags | plist::kSingleLine, key_compare), it.second);
			...
			else
			{
				...
				for(auto const& it : values)
				{
					...
					std::string const& value = std::visit(pretty(flags, key_compare, indent+1, true), it.second);
...
{
	return std::visit(pretty(flags, key_less_than_t(keySortOrder)), plist);
}
